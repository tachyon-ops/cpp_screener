...
async function isAuthorized(from: string): Promise<boolean> {
    try {
...
const candidatesFlow = addKeyword<BaileysProvider, MemoryDB>(['/candidates'])
    .addAction(async (ctx, { flowDynamic }) => {
        if (!(await isAuthorized(ctx.from))) return;
        try {
...
const alertsFlow = addKeyword<BaileysProvider, MemoryDB>(['/alerts'])
    .addAction(async (ctx, { flowDynamic }) => {
        if (!(await isAuthorized(ctx.from))) return;
        try {
...
function connectWebSocket(botInstance: any, provider: any) {
