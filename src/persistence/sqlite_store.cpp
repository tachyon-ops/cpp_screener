...
std::vector<DbPosition> SQLiteStore::get_positions() {
    ...
    if (sqlite3_prepare_v2(pimpl_->db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            pos.direction = dir ? dir : "";
            const char* ets = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4));
            pos.entry_ts = ets ? ets : "";
            ...
            const char* stat = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 9));
            pos.status = stat ? stat : "";
